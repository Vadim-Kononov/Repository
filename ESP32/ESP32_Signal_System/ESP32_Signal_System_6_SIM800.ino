...
if (msgphone.equalsIgnoreCase(my_number) || msgphone.equalsIgnoreCase(boris_number) || msgphone.equalsIgnoreCase(gleb_number))
{
    ...
    else if   (msgbody.equalsIgnoreCase("3"))           bitWrite(situation, 2, 1);
    else if   (msgbody.equalsIgnoreCase("4"))           bitWrite(situation, 2, 0);
    else if   (msgbody.equalsIgnoreCase("5"))           {check_string = String(random(10, 100)); sendSMS(msgphone, "Confirmation, send - " + check_string);}
    else if   (msgbody.equalsIgnoreCase("6"))           {AlarmOFF(); sendSMS(msgphone, "Yes, the Siren is OFF");}
    else if   (msgbody.equalsIgnoreCase(check_string))  {AlarmON();  sendSMS(msgphone, "Yes, the Siren is ON");}
