...
if (TelnetServer.hasClient())
{
  if (!TelnetClient || !TelnetClient.connected())
  {
     ...
     /*Отключение OTA при одключении к серверу*/
     flag_OTA_pass = false;
  }
...
/*Small_Cycle, длительные операции*/
if (flag_Small_Cycle)
{
flag_Small_Cycle = false;


/*Вывод информации о системном времени*/
//DEBUG_PRINTLN(); DEBUG_PRINTLN ("Small_Cycle: " + String(xTaskGetTickCount()/1000.0));

/*Если принятое now_Store[].hash равно отправленной платой Main то прием успешен*/
now_Store[1].hash == now_Store[0].hash ? flag_Now_connected[1] = true : flag_Now_connected[1] = false;
now_Store[2].hash == now_Store[0].hash ? flag_Now_connected[2] = true : flag_Now_connected[2] = false;
...
//xTimerChangePeriod(timerMqttSend, pdMS_TO_TICKS(TACT_time * BOARD_COUNT), 0);
Send_Now ();
...
/*Big_Cycle, длительные операции*/
if (flag_Big_Cycle)
{
flag_Big_Cycle = false;
WiFiReconnect ();
