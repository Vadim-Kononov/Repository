...
{  
  ...
  data_to_send += String(memory.getInt("countReset"));
  data_to_send += F(",");
  data_to_send += String(memory.getInt("countWifi"));
  data_to_send += F(",");
  data_to_send += String(xTaskGetTickCount()/1000.0);
  data_to_send += F(",");
  data_to_send += String(board_count [0]);
  data_to_send += F(",");
  data_to_send += String(board_count [1]);
  data_to_send += F(",");
  data_to_send += String(board_count [2]);
  data_to_send += F(",");
  data_to_send += String("");
  data_to_send += F(",");
  data_to_send += String(""); 
